...
import { CanvasElements, LeaderboardPlayer } from './types.js';

let canvas: HTMLCanvasElement | null, ctx: CanvasRenderingContext2D | null;
let minimapCanvas: HTMLCanvasElement | null, minimapCtx: CanvasRenderingContext2D | null;
let scoreElement: HTMLElement | null, leaderboardContent: HTMLElement | null;
...
export function drawGame(): void {
    ...
    gameState.food.forEach(food => {
        ...
        if (screenX >= -FOOD_SIZE && screenX <= canvasWidth + FOOD_SIZE &&
            screenY >= -FOOD_SIZE && screenY <= canvasHeight + FOOD_SIZE) {
